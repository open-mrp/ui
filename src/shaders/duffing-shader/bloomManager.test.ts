...
describe("BloomManager Performance Tests", () => {
  ...
  describe("Runtime Performance", () => {
    ...
    test("should scale performance with iteration count", () => {
      ...
      expect(results[1].framebuffers).toBeGreaterThanOrEqual(
        results[0].framebuffers
      );
      expect(results[2].framebuffers).toBeGreaterThanOrEqual(
        results[1].framebuffers
      );
  ...
  describe("Integration Performance", () => {
    test("should handle complete bloom pipeline efficiently", () => {
      ...
      const shaders = initBloomShaders(
        mockGL,
        baseVertexShader,
        mockCompileShader
      );
