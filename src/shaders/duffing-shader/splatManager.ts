...
export const applySplatOptimized = (
    ...
    const correctedRadius = correctRadius(config.SPLAT_RADIUS / 100.0, computedAspectRatio);
...
export const applyBatchedSplats = (
    ...
    for (const splat of splats) {
        ...
        gl.uniform2f(splatProgram.uniforms.point, splat.x, splat.y);
        gl.uniform1f(splatProgram.uniforms.radius, correctedRadius);
        gl.uniform3f(splatProgram.uniforms.color, splat.color.r, splat.color.g, splat.color.b);
        blit(dye.write);
    }
    dye.swap();
...
export const handlePointerSplatOptimized = (
    ...
    const speedSquared = pointer.deltaX * pointer.deltaX + pointer.deltaY * pointer.deltaY;
    ...
    const numExtraSplats = Math.min(Math.floor(speed * SPEED_MULTIPLIER), MAX_TRAIL_SPLATS);
 ...
 */
export const correctDeltaX = (delta: number, canvas: HTMLCanvasElement): number => {
 ...
 */
export const correctDeltaY = (delta: number, canvas: HTMLCanvasElement): number => {
    const aspectRatio = canvas.width / canvas.height;
    if (aspectRatio > 1) delta /= aspectRatio;
    return delta;
};
