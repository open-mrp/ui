...
import { applyAdvection, handlePointerSplatOptimized, initSplatShaders } from './splatManager';
import {
    applySunrays,
    applySunraysBlur,
...
import { baseVertexShader, clearShader, copyShader, displayShader } from './shaders';
...
export class FluidRenderer {
    ...
    constructor(
        ...
        const baseVertexShaderCompiled = this.compileShader(gl.VERTEX_SHADER, baseVertexShader);
        ...
        if (shaders) {
            ...
            const copyShaderCompiled = this.compileShader(gl.FRAGMENT_SHADER, copyShader);
            const clearShaderCompiled = this.compileShader(gl.FRAGMENT_SHADER, clearShader);
            const displayShaderCompiled = this.compileShader(gl.FRAGMENT_SHADER, displayShader);
    ...
    private initProgramsWithShaders(
        ...
        this.pressureProgram = new Program(gl, baseVertexShader, compiledShaders.pressure);
        this.divergenceProgram = new Program(gl, baseVertexShader, compiledShaders.divergence);
        this.curlProgram = new Program(gl, baseVertexShader, compiledShaders.curl);
        this.vorticityProgram = new Program(gl, baseVertexShader, compiledShaders.vorticity);
        this.gradienSubtractProgram = new Program(
            gl,
            baseVertexShader,
            compiledShaders.gradientSubtract,
        );
        this.sunraysMaskProgram = new Program(gl, baseVertexShader, compiledShaders.sunraysMask);
        this.sunraysProgram = new Program(gl, baseVertexShader, compiledShaders.sunrays);
        this.blurProgram = new Program(gl, baseVertexShader, compiledShaders.bloomBlur);
        this.bloomPrefilterProgram = new Program(
            gl,
            baseVertexShader,
            compiledShaders.bloomPrefilter,
        );
        this.bloomBlurProgram = new Program(gl, baseVertexShader, compiledShaders.bloomBlur);
        this.bloomFinalProgram = new Program(gl, baseVertexShader, compiledShaders.bloomFinal);
        this.splatProgram = new Program(gl, baseVertexShader, compiledShaders.splat);
        this.advectionProgram = new Program(gl, baseVertexShader, compiledShaders.advection);
        this.colorProgram = new Program(gl, baseVertexShader, compiledShaders.color);
        this.copyProgram = new Program(gl, baseVertexShader, compiledShaders.copy);
        this.clearProgram = new Program(gl, baseVertexShader, compiledShaders.clear);
        this.displayMaterial = new Material(gl, baseVertexShader, compiledShaders.display);
    }
    ...
    private initPrograms(
        ...
        const physicsShaders = initPhysicsShaders(this.gl, baseVertexShader, (type, source) =>
            this.compileShader(type, source),
        );
        const sunraysShaders = initSunraysShaders(this.gl, baseVertexShader, (type, source) =>
            this.compileShader(type, source),
        );
        const bloomShaders = initBloomShaders(this.gl, baseVertexShader, (type, source) =>
            this.compileShader(type, source),
        );
        ...
        const colorShaders = initColorShaders(this.gl, baseVertexShader, (type, source) =>
            this.compileShader(type, source),
        );
    ...
    private initFramebuffers() {
        ...
        const { sunrays: newSunrays, temp: newSunraysTemp } = initSunraysFramebuffers(
            this.gl,
            {
