...
export const initPhysicsShaders = (
  ...
  const compiledPressureShader = compileShader(
    gl.FRAGMENT_SHADER,
    pressureShaderSource
  );
  const compiledDivergenceShader = compileShader(
    gl.FRAGMENT_SHADER,
    divergenceShaderSource
  );
  const compiledCurlShader = compileShader(
    gl.FRAGMENT_SHADER,
    curlShaderSource
  );
  const compiledVorticityShader = compileShader(
    gl.FRAGMENT_SHADER,
    vorticityShaderSource
  );
  const compiledGradientSubtractShader = compileShader(
    gl.FRAGMENT_SHADER,
...
export const applyPressure = (
  ...
  const texelX = texelSize?.x ?? velocity.texelSizeX;
  const texelY = texelSize?.y ?? velocity.texelSizeY;
  gl.uniform2f(programs.pressure.uniforms.texelSize, texelX, texelY);
...
export const applyDivergence = (
  ...
  const texelX = texelSize?.x ?? velocity.texelSizeX;
  const texelY = texelSize?.y ?? velocity.texelSizeY;
  gl.uniform2f(programs.divergence.uniforms.texelSize, texelX, texelY);
...
export const applyCurl = (
  ...
  const texelX = texelSize?.x ?? velocity.texelSizeX;
  const texelY = texelSize?.y ?? velocity.texelSizeY;
  gl.uniform2f(programs.curl.uniforms.texelSize, texelX, texelY);
...
export const applyVorticity = (
  ...
  const texelX = texelSize?.x ?? velocity.texelSizeX;
  const texelY = texelSize?.y ?? velocity.texelSizeY;
  gl.uniform2f(programs.vorticity.uniforms.texelSize, texelX, texelY);
...
export const applyGradientSubtract = (
  ...
  const texelX = texelSize?.x ?? velocity.texelSizeX;
  const texelY = texelSize?.y ?? velocity.texelSizeY;
  gl.uniform2f(programs.gradientSubtract.uniforms.texelSize, texelX, texelY);
  gl.uniform1i(
