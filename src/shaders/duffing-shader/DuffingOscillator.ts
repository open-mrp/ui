export class DuffingOscillator {
    ...
    private sideOfLine(p: [number, number], a: [number, number], b: [number, number]): number {
        return (b[0] - a[0]) * (p[1] - a[1]) - (b[1] - a[1]) * (p[0] - a[0]);
    }

    // Check if a point is inside the boundaries
    private isInBoundaries(p: [number, number]): boolean {
        const s1 = this.sideOfLine(p, this.boundaries.botLeft, this.boundaries.botRight);
        const s2 = this.sideOfLine(p, this.boundaries.botRight, this.boundaries.topRight);
        const s3 = this.sideOfLine(p, this.boundaries.topRight, this.boundaries.topLeft);
        const s4 = this.sideOfLine(p, this.boundaries.topLeft, this.boundaries.botLeft);
    ...
    public update(dt: number): { x: number; y: number; dx: number; dy: number } {
        ...
        if (distSquared > this.cachedMaxDistSquared) {
            ...
            const containmentForce = (0.05 * (dist - this.cachedMaxDist)) / this.cachedMaxDist;
