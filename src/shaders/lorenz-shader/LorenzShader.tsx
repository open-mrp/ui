...
export function LorenzShader({
    ...
    const getRGBColors = (scheme: string) => {
        const config = colorConfigurations[scheme as keyof typeof colorConfigurations];
    ...
    useEffect(() => {
        ...
        try {
            const colors = useDistanceBasedColoring ? undefined : getRGBColors(colorScheme);
            const lorenz = new Lorenz(canvasRef.current, colors, useDistanceBasedColoring);
            ...
            if (useDistanceBasedColoring) {
                const rosolaneToHelvetia = colorConfigurations.rosolane_to_helvetia.gradient;
    ...
    }, [colorScheme, useDistanceBasedColoring, oscillationCenters, isInitialized]);
