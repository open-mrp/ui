...
export class Lorenz {
  ...
  static compile(
    ...
    if (!gl.getShaderParameter(v, gl.COMPILE_STATUS)) {
      throw new Error(
        gl.getShaderInfoLog(v) || "Vertex shader compilation failed"
      );
    }
    ...
    if (!gl.getShaderParameter(f, gl.COMPILE_STATUS)) {
      throw new Error(
        gl.getShaderInfoLog(f) || "Fragment shader compilation failed"
      );
    }
  ...
  private updateDistanceBasedColors(): void {
    ...
    for (let i = 0; i < count; i++) {
      const normalizedDistance =
        range > 0 ? (distances[i] - minDist) / range : 0;
    ...
    gl.bufferSubData(
      gl.ARRAY_BUFFER,
      0,
      this.tail_colors.subarray(0, count * 3)
    );
  }
  ...
  private growBuffers(): void {
    const next2 = (x: number) =>
      Math.pow(2, Math.ceil(Math.log(x) * Math.LOG2E));
    const gl = this.gl;
