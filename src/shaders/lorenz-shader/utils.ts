 ...
 */
export function createLorenzParams(overrides?: Partial<LorenzParams>): LorenzParams {
 ...
 */
export function createDisplayParams(overrides?: Partial<DisplayParams>): DisplayParams {
 ...
 */
export function normalizeColor(color: [number, number, number]): [number, number, number] {
    return color.map((c) => Math.max(0, Math.min(1, c))) as [number, number, number];
}
 ...
 */
export function createColorPalette(hexColors: string[]): [number, number, number][] {
