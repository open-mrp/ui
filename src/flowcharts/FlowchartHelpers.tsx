...
export function createSequenceDiagram({
  ...
  const calculatedRowCount = Math.max(
  ...
  const hasSelfRefInLastRow = actions.some(
    (action) =>
      action.source === action.target && action.row === calculatedRowCount
  );

  // Add 1 to effective row count if there's a self-referencing action in the last row
  const effectiveRowCount = hasSelfRefInLastRow
  ...
  actors.forEach((actor, index) => {
    const xPos =
      actor.position !== undefined ? actor.position : index * actorSpacing;
  ...
  const actionNodes: Node[] = actions.map((action) => {
    ...
    if (!sourceActor || !targetActor) {
      throw new Error(
        `Actor not found: ${!sourceActor ? action.source : action.target}`
      );
    }
    ...
    const width =
      action.width || Math.max(Math.abs(targetX - sourceX) - nodeSpacing, 0);
